...
;(async () => {
  for (const iconSetConfig of config) {
    for (const { dir: frameworkDir, scope, descriptionPrefix, keywords } of frameworks) {
      const scopedPackageName = `@${scope}/${iconSetConfig.name}`
      const scopedPackagePath = path.resolve(__dirname, '..', 'dist', iconSetConfig.name, frameworkDir)
